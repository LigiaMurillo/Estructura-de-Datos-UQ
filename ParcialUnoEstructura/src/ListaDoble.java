...
public class ListaDoble<T> implements Iterable<T> {
	...
	//añadir en cualquier posision
	 public void add(NodoDoble<T> item) {
	        if (nodoPrimero == null) {
	            this.nodoPrimero = item;
	        }
	        else {
	            NodoDoble<T> ultimo = obtenerUltimo();
	          //  if (ultimo != null) {
	                ultimo.setSiguienteNodo(item);
	                item.setAnteriorNodo(ultimo);
	          //  }
	        }
	    }
	 
	 //obtener el ultimo nodo
	 public NodoDoble<T> obtenerUltimo() {
	        NodoDoble<T> next = this.nodoPrimero;
	        while (next != null) {
	            if (next.getSiguienteNodo() == null) {
	                return next;
	            }
	        }
	        return null;
...
//		return(nodoPrimero == null)?true:false;
	...
	public void shell(ListaDoble<Integer> listaEnteros) {
		
        int salto, i, aux1, aux2;
        boolean cambios;
        

        for (salto = listaEnteros.getTamanio() / 2; salto != 0; salto /= 2) {
        	
            cambios = true;
            while (cambios) {   // Mientras se intercambie algún elemento                                         
                cambios = false;
                for (i = salto; i < listaEnteros.getTamanio(); i++)   // se da una pasada
                {
       
                	
                    if (listaEnteros.obtenerValorNodo(i - salto) > listaEnteros.obtenerValorNodo(i)) {
                    	
                    	aux1 = listaEnteros.obtenerNodo(i-salto).getValorNodo();
                    	aux2 = listaEnteros.obtenerNodo(i).getValorNodo();                    
                    	
                    	listaEnteros.modificarNodo(i-salto, aux2);
                    	listaEnteros.modificarNodo(i, aux1);
