...
public class ListaSimpleCircular<T> implements Iterable<T> {
	...
	public void agregarInicio(T valorNodo) {
		...
		{
			NodoUno<T> aux = nodoUltimo.getSiguienteNodo();
...
//				nodoUltimo = null;
	...
	public T eliminarUltimo() {
		...
		if( !estaVacia() ) {			
			...
			NodoUno<T> prev = obtenerNodo(tamanio-2);
			nodoUltimo = prev;
	...
	private NodoUno<T> obtenerNodo(int indice) {
	...
	public void modificarNodo(int indice, T nuevo) {
		...
		if( indiceValido(indice) ) {			
			NodoUno<T> nodo = obtenerNodo(indice);
	...
	public void setNodoPrimero(NodoUno nodoPrimero) {
